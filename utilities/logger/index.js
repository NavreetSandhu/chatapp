...
const namespace= continuation.getNamespace('logger');
import { createLogger, format, transports } from 'winston';
import DailyRotateFile from 'winston-daily-rotate-file';
const appRoot = require('app-root-path');
const logsPath = `${appRoot}/utilities/logger/logs/`;
...
const fileFormat = format.combine(
    ...
    format.printf((info) => {
        let log = ''
        try {
            const logId = (namespace && namespace.get('logId')) ? namespace.get('logId') : uuid();
            const {
